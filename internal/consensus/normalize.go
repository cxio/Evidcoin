...
//   2. 2 倍交易量归一化：按择优池排名升序，后位候选须同时满足 Stakes>2x 且
//      TxCount>2x 才能替换当前最优者。
...
// NormalizeTxVolume 执行 2 倍交易量归一化（第 12 章 §3.2，DEC-0303）：
// 入参 candidates 已完成多签归一化，各块具有不同铸造者。
// 函数按择优池排名升序排列后从最优候选出发，逐个检查后位候选是否满足：
//
//	challenger.Stakes > winner.Stakes * 2  AND  challenger.TxCount > winner.TxCount * 2
//
// 同时满足则以 challenger 替换 winner 并继续；否则停止。
// winner.Stakes==0 或 winner.TxCount==0 时，后位对应指标 >0 即满足（DEC-0303）。
...
func NormalizeTxVolume(candidates []ForkBlock) (winner ForkBlock, ok bool) {
	...
	for _, challenger := range sorted[1:] {
		if txVolumeExceedsDouble(challenger, winner) {
			winner = challenger
		} else {
			// 一旦后位未满足，停止（DEC-0303）。
			break
...
// txVolumeExceedsDouble 判断 challenger 是否同时满足 2 倍约束：
//
//	challenger.Stakes > winner.Stakes * 2  AND  challenger.TxCount > winner.TxCount * 2
//
// winner.Stakes==0 时，challenger.Stakes>0 即满足 Stakes 条件（DEC-0303）。
func txVolumeExceedsDouble(challenger, winner ForkBlock) bool {
	stakesOK := challenger.Stakes > winner.Stakes*2
