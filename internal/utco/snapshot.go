...
// Snapshot 是 UTCO 集在某一区块结算点的不可变快照，承载两类信息：
//
...
type Snapshot struct {
	...
	// StateRoot 是拍照时刻的 UTCO 五层宽成员状态指纹根（见 Store.Root）。
	StateRoot types.TreeHash
	// entries 是拍照时刻状态成员的副本，供回滚还原；不导出以防外部篡改。
	entries map[OutPoint]Entry
}

// Snapshot 对当前 UTCO 集拍照：计算状态 root，复制全部成员，并绑定链身份与
...
// 已转出标记、到期清理）。覆盖使用快照成员的独立副本，故同一快照可重复用于多次回滚。
