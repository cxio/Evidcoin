...
// FlagOutputs 表示一笔交易的凭信输出转移状态位图。
// 每个 bit 对应一个输出，1=未转移，0=已转移。
...
// buildFlagOutputs 根据 entries（同一交易的所有凭信输出）构造状态位图。
func buildFlagOutputs(entries []*UTCOEntry) FlagOutputs {
	if len(entries) == 0 {
		return FlagOutputs{}
	}
	maxIdx := 0
...
// computeDataID 计算某 TxID 下所有凭信输出项数据的哈希摘要。
func computeDataID(entries []*UTCOEntry) types.Hash384 {
	sorted := make([]*UTCOEntry, len(entries))
	copy(sorted, entries)
	...
	for _, e := range sorted {
		...
		buf = appendInt64BE(buf, e.ExpireAt)
...
// groupByTxID 将 entries 按 TxID 分组。
func groupByTxID(entries []*UTCOEntry) map[types.Hash384][]*UTCOEntry {
	groups := make(map[types.Hash384][]*UTCOEntry)
	for _, e := range entries {
...
// ComputeFingerprint 对指定年度的 UTCO 集合计算指纹根哈希。
func ComputeFingerprint(store *UTCOStore, year int) types.Hash256 {
	var yearEntries []*UTCOEntry
...
// ComputeRootFingerprint 对所有年度计算 UTCO 指纹根（跨年汇总）。
