...
// Root 计算当前 UTCO 集的五层宽成员状态指纹根（第 09 章 §2、§5、DEC-0201）：
//
//   - 顶层按年度数值升序，其下三级按 TxID 字节 [7]/[11]/[15] 升序分组，末端按
//     完整 TxID 字典序排列；空年度/空分组不编码。
//   - 各中间层与末端组复用 pkg/hashtree 通用二叉树构造（tree.branch 分支域、
//     奇数层提升、单叶根一元归一化），见 proposal 04 §3.5；末端叶为 utco.leaf
//     域 SHA3-384（见 leafHash）。
//   - 集合为空时返回专用空根 BLAKE3-256(DomainTag("utco.empty"))，非全零，
//     与 UTXO 空根域标签隔离。
//
// 仅未转出输出参与；调用方应先以 ExpireAt 清理过期凭信再计算指纹（UTCO 集语义）。
func (s *Store) Root() types.TreeHash {
	recs := s.leafRecords()
	if len(recs) == 0 {
		return crypto.EmptyUTCORoot()
	}
...
// leafRecords 将所有未转出 entry 按 (年度, 完整 TxID) 归并为末端叶记录；
