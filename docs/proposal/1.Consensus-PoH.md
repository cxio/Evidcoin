...
- 分叉必须延伸至 **29 个区块**（约 150 分钟）后评估。
- 逐高度比较：取 Coinbase 中的铸凭哈希对比。获胜多数（29 中 15+）的分叉成为主链。
- 若分叉在被发现时已超过 29 个区块（隐蔽分叉），则视为独立合法的侧链并被主链忽略。
...
| Fork competition window | 29 个区块（约 174 分钟） | 多数（15/29）胜出 |
...
| Max transaction size | 65,535 字节 | 单笔交易上限 |
