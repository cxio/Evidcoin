...
| Term | Definition |
|------|------------|
| **Minting Transaction** | 参与铸造竞争的历史交易，其可选区块范围为 `[Height-80000, Height-28]`（约 11 个月）。 |
| **Minter** | 铸凭交易**首笔输入来源输出**的接收者公钥地址（即被花费 UTXO 的持有人）。使用已花费地址可避免冷挖矿动机。 |
| **Reference Block** | 高度为 `Height - 9` 的区块，其铸造者的铸凭哈希参与当前区块的铸凭哈希的计算。 |
| **Coin-age Burned** | 高度为 `Height - 27` 的区块的币权总值（聪时数，即聪*小时）参与计算，绑定动态的确定性历史。 |

### 2.2 Anti-Shaping Measures（防塑造措施）

哈希塑造是一种攻击方式，攻击者向哈希输入中注入任意数据以偏向目标输出。PoH 通过精心选择因子来防御：

1. **尾部排除** — 排除区块 `[-1, -27]` 的交易，因为其创建者在创建时已知评参区块参数，可能塑造交易 ID。
2. **区块 ID 排除** — 排除评参区块自身的 ID，因为铸造者可通过 Coinbase 数据与交易排序进行操控。
3. **高成本的币权连锁** — 高成本低可塑性的币权销毁取自 `Height - 27` 区块，此时还不知道评参区块的铸造者，使得币权销毁难以塑造。
...
// stakes:         第 Height-27 区块的币权总值（聪时数，区块头 Stakes 字段）
// blockTimestamp: 当前区块时间戳（由公式计算，非本地时钟）
// privateKey:     铸造者的私钥
func MintingHash(txID Hash384, refMintHash Hash512, stakes uint64,
    blockTimestamp int64, privateKey PrivateKey) Hash512 {

    // 混合常数：增加 Bit 位复杂性
    const Mix = 0x517cc1b727220a95

    // 阶段一：构造动态因子 X，组装源数据并哈希
    // X 将时间戳、币权总值与混合常数结合，可塑性低且成本高昂
    X := encodeInt64(blockTimestamp * int64(stakes) * Mix)
    source := concat(txID[:], refMintHash[:], X)
    hashData := BLAKE3_512(source)

    // 阶段二：用铸造者私钥签名（结果对外不可预测）
    signData := Sign(privateKey, hashData)

    // 阶段三：对签名结果取哈希，得到铸凭哈希（BLAKE3-512）
    return BLAKE3_512(signData)
}
```
...
type MintCandidate struct {
    TxYear    int       // 交易所在年度
    TxID      Hash384   // 铸凭交易 ID（SHA3-384）
    MinterPub PublicKey // 铸造者公钥（首笔输入来源输出的接收者）
    SignData  []byte    // 铸造者对铸凭哈希源数据的签名
    MintHash  Hash512   // 铸凭哈希（BLAKE3-512，由 SignData 计算）
}
```
...
> **注：**
> 择优池的确定性由共约实现，是一种逻辑作用而非精确的算法。
> 这是维护择优池**确定性**的一个极简且无成本的重要机制。
...
type BlockHeader struct {
    Version   int32    // 版本号
    Height    int32    // 区块高度
    PrevBlock Hash384  // 前一区块哈希（SHA3-384）
    CheckRoot Hash384  // Hash(交易哈希树根 + UTXO 指纹 + UTCO 指纹)（SHA3-384）
    Stakes    uint64   // 币权销毁，单位：聪时（聪*小时，不足1小时记为零）
    YearBlock Hash384  // 前一年块哈希（仅当 Height % 87661 == 0 时存在）
}
```

关键字段：

- **CheckRoot**：`Hash384(MerkleRoot(TxIDs) + UTXO_Fingerprint + UTCO_Fingerprint)`。对区块的认可是通过铸造者对区块头内 CheckRoot 的签名（而不是对Coinbase交易签名）。该签名在提交区块时附带，且不参与任何哈希计算，以便将来剪枝剥离。
- **Stakes**：区块内所有交易的币权销毁量总和。币权 = 花费输出金额（聪） × 持有时长（小时，不足 1 小时记为零），单位为「聪时」（聪*小时）。
- **YearBlock**：当 `Height % 87661 == 0` 时，该字段引用上一年块哈希，可在不保存完整区块头的情况下进行长程链验证。

> **注：**
> 铸造者的择优凭证存储在 Coinbase 交易中，且参与交易ID的哈希计算。

### 4.3 Minting Sequence（铸造序列）

当区块时间戳临近：

1. **+30 秒**：排名第 1 的候选者发布区块。在规定时间戳的时间之后 30 秒才发布，以最大化交易收集。
2. **每名 +15 秒**：后续候选者按 15 秒间隔发布（排名 2 在 +45s，排名 3 在 +60s，以此类推）。
3. **提前终止**：若候选者在轮到自己发布前收到并验证了更高排名的区块，则不再发布。
...
2. 铸造者属于当前高度对应的最终 Best Pool 成员。注意检查 Coinbase 交易里的择优凭证，三者（CheckRoot签名者、BestPool 成员、Coinbase择优凭证）皆为同一公钥。
3. 在区块合法的条件下，参考区块竞争规则（§4.5）决定最终接受。
...
| Different minters (redundant publish) | 只有当挑战区块的币权（聪时）销毁量达到领先区块的 **3 倍或以上** 时才获胜（约束交易量操控）。 |
...
- 节点第一次发现分叉时，分叉长度必须 **≤ 20 个区块**，否则不接纳参与主链评比。
- 被接纳的分叉在分叉点之后的 **29 个区块** 内竞争；逐高度比较 Coinbase 中的铸凭哈希。
- 一旦某分叉在链段评比中胜出过半（15 个区块），节点即可提前切换为本链，不必等待 29 个区块全部完成。
- 到 29 个区块仍未提前结束时，胜出更多的一方成为主链。
- 未被接纳参与评比的分叉会被忽略，即便其后续表现更优；这是既成事实的社会性共识边界。

```go
const ForkWindowSize = 29
...
func ResolveFork(main, challenger [ForkWindowSize]Hash512) bool {
    ...
    for i := 0; i < ForkWindowSize; i++ {
        ...
        if wins > ForkWindowSize/2 {
            return true // challenger 已过半胜出，无需继续比较
        }
    }
    return false
}
...
> **注意：**
> 待对比的挑战者的铸凭哈希需要先验证其合法性。

### 5.3 Fork Entry Adjudication（分叉进入裁决）

长度 **20 个区块** 的分叉若临近第 21 个区块铸造时间点才被发现，可能造成部分节点接纳、部分节点拒绝的网络割离。为避免该临界状态，接纳与否由本链当前区块（#21）的择优池前 5 名裁决：

1. 择优池前 5 名成员可自由决定是否接纳该分叉，并签名广播裁决消息。
2. 节点收集裁决消息后，按择优池成员顺序取排名最靠前者的签名作为有效裁决。
3. 如果前 5 名均未签名确认，则默认否决该分叉。

### 5.4 Fork Handling Strategy（分叉处理策略）

1. 仅缓存和评估满足进入条件的分叉。
2. 随新块到达持续评估铸凭哈希。
3. 当某分叉达到多数（15/29）时，宣布其为主链并切换本链。
4. 被放弃分叉中独有的交易被回收进 mempool 重新打包。

### 5.5 Confirmation Safety（确认安全性）

由于分叉竞争仅限最近 29 个区块：

- **确认超过 29 个后续区块（通常表述为 30+ 确认）的交易即具备最终安全性** — 合法分叉窗口已关闭。
- 新币（Coinbase 输出）需 29 个区块确认后方可花费。该规则由锁定脚本中的 `SYS_AWARD` 指令在花费时检查约束条件，不需要额外的分叉协调逻辑。若分叉被放弃，其 Coinbase 输出自然失效，其他交易则可被回收。

### 5.6 Manual Chain Switching（手动链切换）

若网络分区持续超过 2 小时，则 20 区块分叉长度竞争入口关闭，两条链各自成为合法链。此时客户端允许用户通过连接目标分叉的节点并同步分叉后的区块头链来手动选择链。
...
这是一条 **共约**——铸造者最终决定是否纳入低手续费交易。然而，节点 **不得** 阻止低费交易的转播，因为铸造者可能已经将其打包，其他节点需要验证区块。
...
目的：
1. 限定未确认交易池的规模。
2. 为依赖 `Finality` 的应用提供时间确定性。

### 7.4 Time-Skew Delay（错时延迟）— Convention

当交易传播至节点时，若其时间戳显著早于节点当前时间，该交易被称为 **错时交易**（time-skewed transaction）。区块可以收录错时交易（它们属于过去），但这些交易可能诱使铸造者为了多收手续费而延迟出块。

共约规则：

> 当出块时间到达后，时间戳在出块时间之前的错时交易将被 **暂停转播**，直到该高度的区块被创建、广播并确定下来。

此机制使铸造者在出块时间到达后不再收到新的可打包交易，从而消除延迟出块的动机，确保按时铸造。

### 7.5 Zero-Confirmation Warning（零确认警告）

系统不强制零确认安全性。若在 mempool 中检测到双花，将向用户显示警告。对大额交易，用户应等待足够的区块确认。

### 7.6 Network Announcement（全网通告）

保留一个最小化的中心化机制，用于全网文本公告。消息由授权密钥签名（内置于官方客户端或由社区渠道发布），且禁止功能扩展（如可点击的升级链接）。

### 7.7 Coin-Hour Destruction（币权销毁）

币权度量未花费输出的"持有时长权益"，时间单位为**小时**，不足 1 小时记为零（可滤除高频交易影响）：

```go
// CoinStakes 计算币权（聪时 = 聪*小时）。
// @amount:    未花费输出的金额（聪）
// @holdHours: 持有整小时数（不足1小时截断为零）
...
币权销毁在区块头 `Stakes` 字段中汇总，并作为新区块竞争的次级因素（见 §4.5）。

## 8. Design Parameters Summary（设计参数汇总）

| Parameter | Value | Note |
|-----------|-------|------|
| Block interval | 6 分钟 | 固定；时间戳由计算得出，不存储 |
| Blocks per year | 87,661 | ≈365.25 天 × 24h × 60min / 6min |
| Minting tx eligible range | `[Height-80000, Height-28]` | 约 11 个月回溯 |
| Reference block offset | Height - 9 | 用于铸凭哈希计算 |
| Coin-age Burn source | Height - 27 | 深度参与评参区块以防塑造 |
...
| Fork entry limit | ≤ 20 个区块 | 首次发现分叉时的接纳上限；长度 20 的临界分叉需前 5 名择优池成员裁决 |
| Fork competition window | 29 个区块（约 174 分钟） | 多数（15/29）胜出，可提前结束 |
| New coin confirmation depth | 29 个区块 | 花费时校验，由 `SYS_AWARD` 指令实现规则约束 |
| Min fee recalculation period | 6,000 个区块（约 25 天） | 共约，非协议 |
| Transaction expiry | 240 个区块（24 小时） | 协议；严格执行 |
| Hash algorithm (block/tx) | SHA3-384（48 字节） | 用于区块 ID、交易 ID、CheckRoot 等 |
| Hash algorithm (mint hash) | BLAKE3-512（64 字节） | 用于铸凭哈希，速度与信息熵兼顾 |
