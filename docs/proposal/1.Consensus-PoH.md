...
- 新币（Coinbase 输出）需 25 个区块确认后方可花费。该规则由锁定脚本中的 `SYS_AWARD` 指令在花费时检查约束条件，不需要额外的分叉协调逻辑。若分叉被放弃，其 Coinbase 输出自然失效，其他交易则可被回收。
...
| New coin confirmation depth | 25 个区块 | 花费时校验，由 `SYS_AWARD` 指令实现规则约束 |
