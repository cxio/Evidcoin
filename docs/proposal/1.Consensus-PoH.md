...
| **Coin-age Burned** | 高度为 `Height - 27` 的区块的币权总值（销毁币天数）参与计算，绑定动态的确定性历史。 |
...
// stakes:         第 Height-27 区块的币权总值（区块头 Stakes 字段）
// blockTimestamp: 当前区块时间戳（由公式计算，非本地时钟）
// privateKey:     铸造者的私钥
func MintingHash(txID Hash512, refMintHash Hash512, stakes int64,
...
type BlockHeader struct {
    ...
    CheckRoot Hash512  // Hash(交易哈希树根 + UTXO 指纹 + UTCO 指纹)
    Stakes    int      // 币权销毁，按「聪*秒」计算汇总，最终精确到「币*天」
    Height    int      // 区块高度
    YearBlock Hash512  // 前一年块哈希（仅当 Height % 87661 == 0 时存在）
}
```

关键字段：

- **CheckRoot**：`Hash(MerkleRoot(TxIDs) + UTXO_Fingerprint + UTCO_Fingerprint)`。对区块的认可是通过铸造者对区块头内 CheckRoot 的签名（而不是对Coinbase交易签名）。该签名在提交区块时附带，且不参与任何哈希计算，以便将来剪枝剥离。
- **Stakes**：区块内所有交易的币权销毁量总和。币权 = 花费输出金额（聪） × 持有时长（天数），汇总后精确到「币*天」。
...
// @amount:   未花费输出的金额（聪）
// @holdSecs: 持有秒数
func CoinDays(amount int64, holdSecs int) int64 {
    return amount * int64(holdSecs)
}
// 花费后币权归零。
```

币天销毁在区块头 `Stakes` 字段中汇总，并作为新区块竞争的次级因素（见 §4.5）。
