...
- **lead input** 使用完整 64 字节 TxID，便于铸造竞争者（铸凭交易）所有权检索和验证。
...
```graph
Level 0 (Root):    UTXOFingerprint = SHA-512( Year_1 || Year_2 || ... || Year_N )
Level 1 (Year):    YearHash_y      = SHA-512( Tx8Hash_1 || Tx8Hash_2 || ... )
Level 2 (Tx8):     Tx8Hash_t       = SHA-512( Tx13Hash_1 || Tx13Hash_2 || ... )
Level 3 (Tx13):    Tx13Hash_t      = SHA-512( Tx18Hash_1 || Tx18Hash_2 || ... )
Level 4 (Tx18):    Tx18Hash_t      = SHA-512( infoHash_1 || infoHash_2 || ... )

infoHash = SHA-512( TxID || BitBytes || FlagOutputs... )
```
...
type Tx18Node struct {
    Index      byte                 // 分组序位
    Hash       Hash512              // 分组哈希
    InfoHashes []Hash512            // 叶子节点列表（有序）：每个交易对应一个 InfoHash
}
...
```
CheckRoot = SHA-512( TxTreeRoot || UTXOFingerprint || UTCOFingerprint )
```
...
- `HashInputs = SHA-512( LeadHash || RestHash )` ——输入完整性。
- `HashOutputs = SHA-512( BinaryTree( Output_0, Output_1, ... ) )` ——输出完整性。
- `CheckRoot = SHA-512( TxTreeRoot || UTXO Fingerprint || UTCO Fingerprint)` ——区块完整性。
- 默认的哈希算法为 SHA-512，用于 TxID、交易体哈希、区块头哈希、哈希树等。
- UTXO/UTCO 指纹基于应用当前区块交易 **before** 的 UTXO/UTCO 集，采用 SHA3-512 哈希算法以与默认 SHA-512 区分。
