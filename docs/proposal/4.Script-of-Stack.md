...
| `-2`   | 半确定性；特用于 `CALL`，实参数量由其附参在运行时确定，为 `2 + 附参值` |
...
`SCRIPT` 引入的外部脚本是脚本值，主要用于哈希、模式匹配或 `EVAL`；`GOTO` 会创建独立脚本域并执行目标脚本；`EMBED` 则把目标脚本合并进当前脚本域执行。校验域还会暴露 `InGoto`、`InEmbed`、`InScript`、`InCode`、`Gotos` 等状态，供介管脚本判断自身调用场景。
...
| **Function Instructions**（函数指令段） | `[170–224]` | 55 | 常用函数：编码、哈希、签名验证，外加 1 个标准函数引用槽（`FN_X`=224） |
| **Module Instructions**（模块指令段） | `[225–250]` | 26 | 分类模块：数学、时间、正则，外加 1 个标准模块引用槽（`MO_XX`=250） |
| **Extension Instructions**（扩展指令段） | `[251–253]` | 3 | 通用扩展点：`EXT_MO`(251)、`EXT_PRIV`(253)（2 字节附参，各自可寻址 64K）；252 暂未使用 |

Opcode 254 与 255 保留给系统，不属于指令集。

### 3.2 Special Instructions（三个特例指令）

脚本总纲中有三个特例需要在实现和文档中保持醒目：

| 指令 | 特例原因 |
|------|----------|
| `SYS_NULL` | 可在解锁脚本中使用，突破解锁段仅允许 opcode `[0-50]` 的规则；用于标记 `SOURCE` 提取边界。 |
| `SYS_TIME` | 读取节点本地时间，不能用于公共验证共识逻辑；只能在显式 `END` 或隐式 `END`（如公共节点遇到 `INPUT`）之后执行。 |
| `CALL` | 实参数量为 `2 + 附参值`，运行时可确定但静态配置无法固定，因此使用 `-2` 半确定性参数规则。 |

### 3.3 Base Instruction Categories（基础指令分类）

170 条基础指令按功能分类：

| 类别 | 范围 | 数量 | 关键指令 |
|----------|-------|-------|-----------------|
| Value Instructions（值指令） | `[0–18]` | 19 | `NIL`, `TRUE`, `FALSE`, `DATA{}`, `CODE{}`, `SCRIPT`, `VALUE{}`, 整数/浮点/字符串/正则/日期字面量 |
| Capture Instructions（截取指令） | `[19–23]` | 5 | `@`（实参捕获），`~`（直接栈），`$`（本地存储），`$()`（本地读取），`$X()`（循环变量） |
| Stack Operations（栈操作指令） | `[24–34]` | 11 | `NOP`, `PUSH`, `POP`, `TOP`, `PEEK`, `SHIFT`, `CLONE`, `VIEW` |
| Collection Operations（集合指令） | `[35–45]` | 11 | `SLICE`, `REVERSE`, `MERGE`, `EXTEND`, `PACK`, `SPREAD`, `SET`, `ITEM`, `INDEX`, `CALL`, `SIZE` |
| Interaction Instructions（交互指令） | `[46–50]` | 5 | `INPUT`, `OUTPUT`, `BUFDUMP`, `PRINT` |
| Result Instructions（结果指令） | `[51–57]` | 7 | `PASS`, `CHECK`, `GOTO`, `EMBED`, `EXIT`, `RETURN`, `END` |
| Flow Control（流程控制） | `[58–66]` | 9 | `IF{}`, `ELSE{}`, `SWITCH{}`, `CASE{}`, `DEFAULT{}`, `EACH{}`, `CONTINUE`, `BREAK`, `BLOCK{}` |
...
| Environment Instructions（环境指令） | `[128–137]` | 10 | `ENV{}`, `IN{}`, `Out{}`, `INOUT{}`, `XFROM{}`, `VAR`, `SETVAR`, `SOURCE`, `SIGNED` |
| Tool Instructions（工具指令） | `[138–163]` | 26 | `EVAL`, `COPY`, `MATCH`, `RANGE`, `MAP{}`, `FILTER{}`, `SHELL{}`，+ 8 保留（量子安全） |
| System Instructions（系统指令） | `[164–169]` | 6 | `SYS_TIME{}`, `SYS_AWARD`, `SYS_CHKPASS`, `SYS_NULL` |

### 3.4 Instruction Naming Conventions（指令命名约定）
...
`GOTO` 将执行重定向到另一笔交易的输出脚本。目标脚本运行于 **independent sub-environment**，拥有独立的数据栈、实参区与全局变量，但与调用者共享 INPUT/OUTPUT 缓存区（**注**：INPUT/OUTPUT 两者之间并不互通）。
...
| 指令 | 描述 |
|-------------|-------------|
| `FN_HASH224(algo)` | 哈希到 224 位（BLAKE3/BLAKE2/SHA2/SHA3） |
| `FN_HASH256(algo)` | 哈希到 256 位（BLAKE3/BLAKE2/SHA2/SHA3） |
| `FN_HASH384(algo)` | 哈希到 384 位（BLAKE3/BLAKE2/SHA2/SHA3） |
| `FN_HASH512(algo)` | 哈希到 512 位（BLAKE3/BLAKE2/SHA2/SHA3），链上默认 |
...
哈希函数的算法由具体函数指令的附参定义。当前函数指令提案中，`FN_HASH224/256/384/512` 的默认算法标识为 `0`，即 BLAKE3；SHA3 使用算法标识 `3`；BLAKE2 实为 BLAKE2b 算法。
...
```go
// 导出（非阻塞）
@POPS[3] OUTPUT     // 弹出栈顶3项 → 导出到 OUTPUT 缓存区
BUFDUMP[tag]        // 触发外部监听器，清空 OUTPUT 缓存区

// 导入（阻塞）
INPUT[3]            // 从 INPUT 缓存区取3项 → 入栈
                    // 缓存区无值或数量不匹配：正常失败（不影响调用者 PASS 状态）
```

**Export** 为非阻塞：有临时缓存区，可接受多次 `OUTPUT` 积累数据，遇到 `BUFDUMP` 时触发外部监听器并清空缓存。外部应注册监听，由监听中间件接手处理链下业务逻辑。

**Import** 为阻塞：若缓冲区为空或数量不匹配，执行以可恢复方式失败（不影响调用者 PASS 状态）。对公共校验节点，`INPUT` 等同隐式 `END`，因为这些节点不会注入任何外部数据。

> **注意：**
> INPUT/OUTPUT 连接的是两个各自独立的通道，两个缓存区并不互通，它们主要服务于链外事务，而不是 GOTO/EMBED 外部脚本与调用者的通道（比如 GOTO 的脚本 OUTPUT， 调用者 INPUT，或反之）。
> BUFDUMP 监听是私有的，只有需要的节点才会部署。

### 4.8 Mediator Script Check（介管脚本校验）

介管脚本常通过 `GOTO` 被主脚本调用。它不仅执行自身逻辑，还可以用环境指令和模式匹配验证调用者结构，避免伪造调用。

```go
ENV{InGoto} PASS          // 必须由 GOTO 进入
XFROM{Source}             // 取得调用源脚本片段（具体条目见环境指令）
MODEL {
    ...
    ^GOTO[?, ?, ?]        // 验证源脚本确实包含目标 GOTO
    ...
}
PASS
END                       // 公共验证在此结束

@OUTPUT BUFDUMP           // 私有监听逻辑，可由中间件处理
```

`END` 是公共验证与私有业务逻辑的边界。公共节点在 `END` 处停止；部署了私有监听的节点可继续执行后续 `OUTPUT`/`BUFDUMP` 等链外逻辑。
...
│    - Restricted to opcodes [0-50], except SYS_NULL(169)  │
...
├─────────────────────────────────────────────────────────┤
│ 4. Check pass-state                                     │
│    - PASS: validation succeeds, UTXO/UTCO can be spent  │
...
解锁脚本仅允许 opcode `[0–50]`，并例外允许 `SYS_NULL`（opcode 169）：
- 值指令（将数据入栈）
- 截取指令（实参区操作）
- 栈操作指令（重排）
- 集合指令（基础结构处理）
...
标准签名、授权标记、公钥等解锁数据可由执行环境载入并供 `SYS_CHKPASS` 消费，不必进入普通数据栈。这保持签名数据与 TxID 计算解耦。
...
| 解锁脚本 opcode | 仅 `[0–50]`，例外允许 `SYS_NULL`(169) | 防止绕过攻击 |
| `SYS_TIME` | 仅 `END` 或隐性 `END(INPUT)` 之后 | 节点本地时间不确定，不能参与公共验证 |

> **注意：**
> 对于锁定脚本的长度限制检查包含两项：
> 1. 作为输入项的引用源输出的锁定脚本。这实际上是一种冗余检查。
> 2. 当前交易中每个输出项的锁定脚本。超过限制的交易将被拒绝（入门把关）。
...
| 第 4–12 个月 | 每月 +1 MB（最多 10 MB） |
| 第 2 年起（87,661 区块/年） | 每年 +1 MB |

区块容量包含解锁部分（但不含标准签名数据）。
...
- `EXT_PRIV`（私有扩展）仅用于私有应用逻辑；公共验证逻辑不应抵达该指令。
- `SHELL{}`（嵌入 Shell 代码）只在私有逻辑中生效；公共验证逻辑不应抵达这些能力指令。
...
### Source format example（源码格式示例）:

```go
SYS_CHKPASS
ENV{Height} {<expireTime>} GT
IF {
    SIGNED[0] PASS
    EXIT
}
...
| **Time-locked redemption** | `ENV{Height}` + `IF{}` + `SIGNED` |
...
| **Attestation identification** | `SHELL{}` for programmatic recognition |
...
3. **Four-segment extensible architecture** 使指令集从 170 条基础指令扩展到理论上无限（函数指令 55 个，模块指令 26 个，扩展指令 3 个）。
4. **Pattern matching**（`MODEL{}`）实现无信任跨脚本结构校验，是跨链与自动化流程的独特能力。
5. **Strict resource bounds** 确保确定性执行并防止拒绝服务攻击。
6. **Clean separation** 在解锁数据（受限 opcode `[0–50]`，例外允许 `SYS_NULL`）与锁定逻辑（完整指令集）之间保持清晰分离，维持安全不变量。
