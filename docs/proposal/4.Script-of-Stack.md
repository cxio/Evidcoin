...
| **Function Instructions**（函数指令段） | `[170–224]` | 55 | 常用函数：编码、哈希、签名验证，外加 1 个自扩展槽（`FN_X`=224） |
| **Module Instructions**（模块指令段） | `[225–250]` | 26 | 分类模块：数学、时间、正则，外加 1 个自扩展槽（`MO_XX`=250） |
...
│    - Restricted to opcodes [0-50,169]                       │
...
- `EXT_PRIV`（私有扩展）被公共校验节点忽略——仅用于私有应用逻辑。
...
6. **Clean separation** 在解锁数据（受限 opcode [0–50, 169]）与锁定逻辑（完整指令集）之间保持清晰分离，维持安全不变量。
