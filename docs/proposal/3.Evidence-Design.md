...
- 数量以最小单位 `chx`（同 `Satoshi` 聪）的变长整数表示。
...
type CoinOutput struct {
    Receiver   []byte  // 接收者地址（公钥哈希，32 字节）
    Amount     int64   // 币金数量，最小单位 chx（同 Satoshi 聪）
    Memo       []byte  // 附言，可选，最长 255 字节
    LockScript []byte  // 锁定脚本，最长 1024 字节
}
```

如果用户不使用内置的 `SYS_CHKPASS` 标准验证，接收者可为空或任意小于 256 字节的字节序列。
...
type AttachmentID struct {
    ...
    Fingerprint    [64]byte // 附件指纹，SHA3-512 哈希（完整附件数据）
    ShardCount     uint16   // 分片数量（< 65536）
    ShardTreeRoot  [32]byte // 片组哈希树根（BLAKE3-256，条件存在）
...
- **Fingerprint:** 附件指纹是对数据本身的完整 SHA3-512 哈希，不分片，固定 64 字节。
- **Hash algorithm:** 附件指纹使用 SHA3-512——确保超长期安全，与分片树根哈希形成双哈希耦合保护（不同算法）。
- **ShardTreeRoot:** 片组哈希用于分片验算，与附件指纹的逻辑不同。双哈希可强化附件安全性（采用不同的哈希算法）。
- **Shard tree:** 二元哈希树（类 Merkle）。每个叶子为分片数据的 32 字节哈希，前缀 2 字节序号（每叶共 34 字节）。树内部节点采用 BLAKE3-256 计算。
- 若单片最大 1 MB、最多 65,535 片时，附件可表示约 64 GB 数据。
...
| `1` | 逻辑上未分片。`ShardTreeRoot` 正常计算，但没有树形分支。 |
...
常见情形：一个或多个简单私钥（单一公钥对应单一私钥）签名交易以授权花费。
...
对于同一输入源的公钥哈希（复合型），支持 `M-of-N` 方案，以提升社会化安全与灵活性。
...
// 多重签名地址构造流程
// 1. 对 N 个公钥分别计算双重哈希
//    PKH_i = SHA3_256( BLAKE2b_512( pubKey_i ) )
// 2. 公钥哈希有序串连，前置 m/N 配比
//    PKHs = SHA3_256( BLAKE2b_512( [m, N] || PKH_1 || PKH_2 || ... || PKH_N ) )
// 3. PKHs 即为复合型公钥哈希，将其编码为文本形式即为多签地址
```
...
1. 从解锁数据规模推导 `m` 与 `N`。
2. 对每个提供的公钥计算哈希：`PKH_i = SHA3_256(BLAKE2b_512(pubKey_i))`。
...
用*1字节*标志配置签名覆盖的交易部分，从而支持灵活的授权模式。
...
// 签名数据不参与任何哈希计算，因此不影响 TxID。
```

### 7.2 Rationale（设计理由）

1. **Malleability resistance:** 签名不参与 TxID，第三方无法通过修改签名改变 TxID（消除早期比特币的交易可塑性问题）。
2. **Signature discardability:** 签名在区块纳入时验证。一旦区块确认，安全性将由三链耦合（区块 ID 链 + UTXO/UTCO 双指纹链）保证，签名可丢弃以回收存储。
3. **Long-term security:** 签名被丢弃后，链的完整性依赖三链耦合机制而非持久签名。即便未来签名算法被攻破，链历史仍不可篡改，具备后量子韧性。

### 7.3 Multi-Chain Coupling（多链耦合）

长期链完整性由三条互锁的哈希链维持：

- **Block-ID Chain (区块 ID 链):** 每个区块头包含前一区块哈希，形成顺序链。
- **UTXO/UTCO Fingerprint Chain (UTXO/UTCO 双指纹链):** 每个区块提交累积 UTXO 和 UTCO 状态，形成并行完整性链。

三条链共同提供密码学证明，确保区块及其交易内容未被篡改，且不依赖签名数据。
...
- **Algorithm:** `SHA3-256( BLAKE2b-512 )`，即 `SHA3-256( BLAKE2b-512( publicKey ) )`，输出 **32 字节**。
- **Rationale:** 采用双重哈希，兼顾后量子安全与数据量节省。该设计提供约 128 位量子安全强度，是空间节省与长期安全之间的有意识权衡；未花费或未转移输出仍会让地址在链上持续暴露，后续实现应避免夸大其安全裕度。

```go
const PubKeyHashLen = 32 // SHA3-256(BLAKE2b-512) 输出长度（字节）

// PubKeyHash 计算公钥哈希。
// pkh = SHA3_256( BLAKE2b_512( publicKey ) )
type PubKeyHash [32]byte
```
...
```graph
 [Coinbase Minting] ──► [UTXO Set] ──► [Spent as Input] ──► [New Outputs]
                            │
                            └──► [Destroy Flag] ──► [Disappeared from UTXO]
```
...
```graph
 [Creation (NEW flag)] ──► [UTCO Set] ──► [Transfer] ──► [UTCO Set (new owner)]
      ...
      │                         │               └──► [Revision if MUTABLE]
      │                         │
      │                         ├──► [Destroy Flag] ──► [Disappeared from UTCO]
      │                         │
      │                         └──► [Expiry / Transfer-count exhausted]
      │                                  │          |
      │                                  │          └──► [Disappeared from UTCO]
      │                                  └──► [Removed from UTCO]
      │
...
4. **Expiry:** 通过一个额外的 `高度=>[Index]` 的引用索引集，当抵达某个高度时，对 UTCO 集进行清理或设置（对于单项）。
...
#### Credit Lifecycle Constraints（凭信生命周期约束）

为约束 UTCO 集的规模、防止滥用或无限增长，Credit 受以下协议级限制：

| Constraint | Value | Description |
|---|---|---|
| **Max Expiry Height** | 100 年（相对高度差） | 高度截止不能超过约 100 年（`100 × 87661 = 8,766,100` 个区块）。 |
| **Activity Requirement** | 11 年 | 无期限的 Credit 至少每 11 年激活一次（可转给自己），否则失效并从 UTCO 集移除。此为 **协议**，强制执行以维护 UTCO 指纹的一致性。 |
| **Transfer Count** | < 65536 | 由 2 字节容量自然限定，且依然受 11 年活跃保证约束。 |

> **注：**
> 设置了高度截止的 Credit（因有交易费成本）**不受** 11 年活跃度约束——其生命周期由截止高度确定性地终结。
...
- 公钥哈希使用 SHA3-256(BLAKE2b-512)（32 字节），减少 UTXO 条目尺寸。
- Credit 次数归零采用被动检查——仅在转移时判断，无需扫描。
- Credit 高度截止通过索引集指向局部清理，避免全量扫描 UTCO 集。
- Proof 从不进入 UTCO 或 UTXO 集，仅展示客观存在。
...
- 签名分离消除交易可塑性。
- 多链耦合（区块 ID 链 + UTXO/UTCO 双指纹链）提供独立于签名的长期完整性。
- 附件指纹使用 SHA3-512（64 字节）提供超长期安全强度。
- 附件分片的哈希树叶子节点为 2 字节序号 + 32 字节分片哈希，内部分支节点使用 BLAKE3-256，兼顾安全性与性能。
- 公钥哈希使用 SHA3-256(BLAKE2b-512) 提供约 128 位量子安全强度，是地址长度与长期安全之间的权衡。
