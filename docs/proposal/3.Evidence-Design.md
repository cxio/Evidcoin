...
如果用户不使用内置的 `SYS_CHKPASS` 标准验证，接收者可为空或任意32字节。
...
type AttachmentID struct {
    ...
    Fingerprint    []byte   // 附件指纹，SHA3-512 哈希（最低 128 bit / 16 字节）
...
- **Shard tree:** 二元哈希树（类 Merkle）。每个叶子为分片数据的 48 字节哈希（SHA3-384），前缀 2 字节序号（每叶共 50 字节）。树内部节点采用 BLAKE3-256 计算。
...
| `1` | 逻辑上未分片。`ShardTreeRoot` 等于完整附件的 SHA3-384 哈希（48 字节）。 |
...
- **Rationale:** 采用双重哈希，兼顾后量子安全与数据量节省。UTXO/UTCO 条目仅在花费前存在，256 位安全强度足以覆盖其生命周期，同时比单次 SHA3-384 节省 16 字节。
...
- 附件分片的哈希树叶子节点使用 SHA3-384，内部分支节点使用 BLAKE3-256，兼顾安全性与性能。
- 公钥哈希使用 SHA3-256(BLAKE2b-512) 提供 128 位安全强度，足以覆盖短暂的 UTXO/UTCO 生命周期。
