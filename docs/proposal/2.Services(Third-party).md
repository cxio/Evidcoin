...
- **Scarcity sensing**：当广播一个数据查询时，持有目标数据的节点会响应并停止继续转发；不持有数据的节点会将跳数计数器加一并继续转发。跳数可粗略衡量数据稀缺程度——跳数越高表示越稀缺，从而为存储决策提供依据。**注**：应用节点可能因某种原因（如隐藏起点）将请求的起跳数设为非零值，这是允许的。评估节点如需获取真实距离，可自行发起一个从零起跳的请求进行独立探测。
...
**Scope boundary**：小于 10 MB 的附件或大附件的分片索引文件也可由 Blockqs 提供；完整区块与大于 10 MB 的附件由 Depots 文件分享服务提供。
...
| **UTXO Data** | 获取特定年度或当前UTXO集数据 |
| **UTCO Data** | 获取特定年度或当前UTCO集数据 |
...
### 3.2 Formal Issuance Phase（正式发行阶段） — Starting Year 4
...
## 4. Deflation Mechanism（通缩机制）

### 4.1 The 50% Fee Burn Rule（50%费用燃烧规则）

交易手续费是交易中输入币值与输出币值之差。Evidcoin 对所有交易手续费施行 **50% burn**：

- 每个区块的交易手续费总额中，仅 **50%** 记入 Coinbase 输出。
- 其余 **50%** 永久销毁（burn），创建为一个输出项，设置销毁标记（接收者为 `null`）。

### 4.2 Dynamic Equilibrium（动态平衡）
...
## 5. Revenue Distribution（收入分配）

### 5.1 Coinbase Transaction Structure（铸币交易结构）
...
### 5.3 Coinbase Outputs（Coinbase 输出项）
...
### 6.3 Early Redemption（提前兑现）

若某奖励在 48 区块窗口结束前已获得 2 次确认，则在 **29 blocks** 后（分叉风险已消除）即可兑换。

> **注：**
> 此规则由锁定脚本中的 `SYS_AWARD` 指令实现。

### 6.4 Evaluation Methods（评估方法）
...
### 6.5 Cache Pool for Evaluation（评估缓存池）
...
## 7. Long-term Sustainability Analysis（长期可持续性分析）

### 7.1 Revenue Guarantee（收益保障）
...
### 7.3 The "Stingy Blockchain" Consideration（吝啬的区块链）
...
### 7.4 Generality of Services（服务通用性）
...
### 7.5 Security Considerations（安全性考虑）
