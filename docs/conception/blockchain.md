...
- 1币 = 100,000,000 聪。1天 = 86400 秒。
- 1币天 = 8640,000,000,000（100000000 * 86400）聪秒。
...
因此一个合适的哈希算法采用策略是必要的，也是很重要的。

- **区块头**：`SHA2-512`。区块头内数据无自由灵活度，传统哈希足够。
- **CheckRoot**: `SHA3-512`。Hash512(`TreeRoot || UTXO || UTCO`)，一次性开销，成本可控。
- **树结构**：`BLAKE3`。高性能的现代算法，处理需多次哈希的场景。
- **交易头**：`SHA3-512`。同样为一次性，且更现代。
- **附件指纹**：`SHA3-512`。需确保长期安全，且可与附件的片组哈希（树结构：`BLAKE3`）形成耦合保护。
- **公钥哈希**：`SHA3-384`。即区块链账户地址，动态花费变换，可适量节省。


### 哈希树结构

- UTXO 集指纹计算：`BLAKE3-512`。
- UTCO 集指纹计算：`BLAKE3-512`。
- 区块交易集的二元哈希树：`BLAKE3-512`。
- 交易输入集/输出集的哈希树：`BLAKE3-512`。
- 附件片组集的哈希树：`BLAKE3-384`，有附件指纹（`SHA3-512`）耦合保护。


### 小结

本系统采用了共**3**种哈希算法：`SHA2/SHA3/BLAKE3`。其中性能较弱的 `SHA2/SHA3` 用于单次哈希计算，高性能的 `BLAKE3` 则用于树结构的多哈希场景。

传统 `SHA2` 算法使用场景最少，仅区块头和指令集中的 `FN_HASH224, FN_HASH256...` 系列支持。
