...
- Coinbase 头字段顺序（DEC-0003）：`Version(uint16) || HashOutputs[32] || Timestamp(int64) || MintPKHash[32]定长 || BlockHeight(uint32) || Minter(MintProof) || FreeData(varint(len)||bytes, len≤255) || BurnCoin(int64)`；**无 `HashInputs` 字段**。
