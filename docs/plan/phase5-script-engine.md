...
1. **VALUE** 指令：从环境读取值。这是一个确定的目标值集合（具体条目待定），逻辑类似于环境指令，但解锁脚本可读。
2. **DICT** 指令：由两个实参构造而来，第一个实参为键列表（[]string），第二个实参为值列表（[]any）。DICT 指令会将这两个列表按索引一一对应构造出 map[string]any。**即**：`<keys> <values> DICT`；或 `<keys> <values> @POPS[2] DICT`，键值对先弹出到实参区（但无必要）。
3. **PACK** 指令：各类型的二进制编码格式遵循 Go 原生编码方式，每条数据前置类型码（1字节）。例如，int64 使用 8 字节 big-endian 编码；字符串使用 UTF-8 编码（前置类型码，无需长度定义）；ValueSet 内部的值按其类型分别编码。**注**：极简化紧凑封装，无需考虑 unpack 到原始输入（但可 unpack 到基本类型序列）。
4. **SCRIPT** 指令的链上加载：预留接口 stub，实际链上访问逻辑在 Phase 5 实现核心后补充。
5. **SYS_AWARD**：仅实现接口 stub，具体铸造验证逻辑与 PoH 共识紧密耦合，待 Phase 6 实现。
