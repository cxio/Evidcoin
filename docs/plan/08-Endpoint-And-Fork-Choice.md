...
- 从当前最优 `winner` 起考察后位 `challenger`；`challenger` **同时**满足 `challenger.Stakes > winner.Stakes×2` 且 `challenger.TxCount > winner.TxCount×2` 时替换并继续；否则停止。
- `TxCount` 含 Coinbase；相等不算超越（必须严格 `>2x`）；`winner.Stakes==0` 或 `winner.TxCount==0` 时仍按公式（后位指标 `>0` 即满足 `>2x`）。
