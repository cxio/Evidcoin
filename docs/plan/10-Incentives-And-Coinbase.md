...
- 百日前（`height <= 24000`）2 输出：铸凭者 20%、校验组 80%（重标定）；无公共服务奖励、不使用 `SYS_AWARD`、`AwardSlots` 全零或空槽编码。
...
- Coinbase 头携带 `AwardSlots [18]byte`（不作为输出项）；Blockqs/Depots/STUN 各 **6** 字节，顺序为 Blockqs 6B、Depots 6B、STUN 6B。
