...
Conception 已明确 UTXO/UTCO 指纹按年度、TxID 字节索引 `[7,11,15]` 分层，末端叶为 `Hash384(TxID || FlagOutputs)`。该索引组取代旧讨论中的 `[8,13,18]`：交易输入项 `TxIDPart` 最短只保证前 16 字节可用（见 DEC-0101），旧索引 `[18]` 会落到最短引用覆盖范围之外，导致无法仅凭最短引用有效检索 UTXO/UTCO 目标分组。仍需冻结状态位顺序、空分组哈希、年度排序和缓存详情边界。
...
- 顶层年度按数值升序排列。
- 年度内按 TxID 字节索引 `[7,11,15]` 分入三级宽成员节点；索引为 0-based，分别对应 TxID 第 8、12、16 个字节。
...
- 实现和测试必须以 0-based `[7,11,15]` 作为唯一分层索引，测试向量应覆盖第 8、12、16 个字节。
