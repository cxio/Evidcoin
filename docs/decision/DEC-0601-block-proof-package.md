...
- 验证 `CoinbaseTx.Minter` 的铸凭证明：`Nonce >= BlockHeader.Height`、`Solution` 可解析且索引升序、Equi-X 校验通过，并且重算 `MintHash` 与签名匹配（此步应当在择优池成员进入时已验证，可略过重验）。
...
- 测试需要覆盖择优池成员检查、前一区块衔接、UTXO/UTCO 本地不一致、Coinbase 非 0 序位、TreeRoot 不匹配、MintProof `Nonce` 非法、`Solution` 非升序、MintProof 签名错误和 CheckRoot 签名错误。
