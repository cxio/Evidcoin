...
| **Coin-age Burned** | 高度为 `Height - 24` 的区块的币权总值（销毁币天数）参与计算，绑定动态的确定性历史。 |
...
3. **高成本的币权连锁** — 高成本低可塑性的币权销毁取自 `Height - 24` 区块，此时还不知道评参区块的铸造者，使得币权销毁难以塑造。
...
// stakes:         第 Height-24 区块的币权总值（区块头 Stakes 字段）
...
| Coin-age Burn source | Height - 24 | 深度参与评参区块以防塑造 |
