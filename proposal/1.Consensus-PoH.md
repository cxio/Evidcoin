...
| **UTXO/UTCO Fingerprint** | 表示某个区块的交易应用后，全链 UTXO/UTCO 集状态的摘要。 |
...
- **CheckRoot**：`Hash(MerkleRoot(TxIDs) + UTXO_Fingerprint)`。区块头中的“签名”是铸造者对该 CheckRoot 的签名，在提交区块时附带提供（不参与 Coinbase 交易ID的哈希计算，可在将来被剪枝剥离）作为铸造凭证。**注**：铸造者的择优凭证存储在 Coinbase 交易中，参与交易ID的哈希计算。
