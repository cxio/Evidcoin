...
- **Opcode**: 1-byte instruction identifier (0–254 usable).
...
The instruction set is organized into four contiguous segments within a single-byte opcode space (0–254):
...
3. The target script's PASS/FAIL result is written back to the caller's pass-state.
...
| Max count | ≤ 2 at main script level | ≤ 5 total (runtime count) |
...
| `FN_ADDRESS` | Encode public key hash to human-readable address (Base32) |
...
| Unlock script length | < 4 KB | Allows sufficient unlock data (signatures, proofs) |
| Transaction size | < 8 KB | Excluding unlock portion |
| Output count per tx | < 1024 | Single-byte 7-bit expression (128×8) |
| EMBED invocations | ≤ 5 | Runtime count, including sub-script embeds |
