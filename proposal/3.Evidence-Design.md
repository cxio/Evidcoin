...
**Special address — `null`:** 向 null 地址支付会销毁币金。发送到 null 的输出不进入 UTXO 集。
...
| 10 | `HAS_EXPIRY` | 后接 4 字节过期时长（秒）。 |
| 0–9 | `DESC_LEN` | 描述内容长度（< 1024 字节）。 |

转移次数与过期可并存；先触及的限制会终止该 Credit。过期的 Credit 会从 UTXO 集移除，但仍保留在链上（类似 Proof）。
...
|--------|------|-------------|
| 11–15 | (reserved) | — |
| 0–10 | `CONTENT_LEN` | 内容字节长度（最大 2048）。 |

由于 Proof 不可转移，IdentScript 与锁定脚本用途不同——它使第三方客户端能通过脚本跳转指令（如 `GOTO`）对 Proof 进行程序化识别与处理。
...
| 5 | `Destroy` | 销毁 Coin 或终止 Credit 的可转移性。输出不进入 UTXO。 |
| 4 | (reserved) | — |
...
**Custom Class (自定义类):** 启用私有应用专用脚本与扩展指令。自定义输出不能作为 UTXO 输入。最多 127 字节的私有标识用于锚定专用客户端。
...
- **Shard tree:** 二元哈希树（类 Merkle）。每个叶子为分片数据的 48 字节哈希，前缀 2 字节序号（每叶共 50 字节）。序号前缀由验证者施加，防止源节点篡改。
- 单片最大 2 MB、最多 65,535 片时，附件可表示约 128 GB 数据。

### 4.2 Shard Verification（分片验证）

在不下载完整附件的情况下验证单个分片：

1. 向源节点请求分片数据及其 Merkle 证明路径。
2. 计算接收分片数据的 BLAKE3 哈希。
...
| `ShardCount` | 含义 |
|---|---|
| `0` | 无分片。`ShardTreeRoot` 全零（无意义）。 |
| `1` | 逻辑上未分片。`ShardTreeRoot` 等于完整附件的 BLAKE3 哈希。 |
...
| 类别 | 规则 | 示例 |
|----------|------|---------|
| 独立项 | 可单独使用或与其他组合。 | `SIGIN_ALL` |
| 主项 | 必须与辅项组合。 | `SIGOUT_ALL \| SIGRECEIVER` |
| 辅项 | 必须与主项组合。 | `SIGOUT_SELF \| SIGOUTPUT` |

**Most common configuration:** `SIGIN_ALL | SIGOUT_ALL | SIGOUTPUT` — 覆盖所有输入与全部输出的完整数据。
...
2. **Signature discardability:** 签名在区块纳入时验证。一旦区块确认，并由双链耦合（区块 ID 链 + UTXO 指纹链）固定位置，签名可安全丢弃以回收存储。
...
- **UTXO Fingerprint Chain (UTXO 指纹链):** 每个区块提交累积 UTXO 状态，形成并行完整性链。
...
- **Algorithm:** SHA-384（48 字节）。
- **Rationale:** 相较 SHA-512 更短，因为 UTXO 条目是短暂的——仅在花费前存在。384 位安全强度足以覆盖 UTXO 生命周期，同时节省频繁访问的 UTXO 集存储。
...
4. **Destruction:** 设置 `Destroy` 标记或支付至 `null` 地址会永久移除 Coin 流通。
...
2. **Transfer:** 类似花费 Coin。Credit 从 UTXO 集消耗并以新持有人重建。
    - 若 `MUTABLE`：描述可修改，必须置 `MODIFIED` 标记。
    - 若不可变：仅接收者与锁定脚本可变更；创建者字段转为引用。
3. **Mutability transition:** Credit 可从可变变为不可变（单向且不可逆）。
4. **Expiry:** 在转移校验时被动检查。达到过期时间或转移次数上限时，从 UTXO 集移除；过期 Credit 仍保留为链上历史数据。
5. **UTXO cleanup:** 源交易的所有输出被花费后，关联的过期 Credit 会从 UTXO 集彻底移除。
...
3. **No UTXO presence:** Proof 不进入 UTXO 集（不可花费）。
4. **Referencing:** 其他交易脚本可通过 IdentScript 使用跳转指令（如 `GOTO`）引用 Proof，使第三方客户端可程序化发现。
...
| 可分割 | 是 | 否 | 否 |
| 可转移 | 是（花费） | 是（转移） | 否 |
| 在 UTXO 集 | 是 | 是（直到过期/销毁） | 否 |
| 链上永久性 | 是 | 是 | 是 |
| 可销毁 | 是（null/Destroy） | 是（Destroy/过期） | 不适用 |
...
- 公钥哈希使用 SHA-384（48 字节）而非 SHA-512（64 字节），减少 UTXO 条目尺寸。
- Credit 过期采用被动检查——仅在尝试转移时判断，避免无谓扫描。
- Proof 从不进入 UTXO 集，保持其紧凑。
...
- 签名分离消除交易可塑性。
- 双链耦合（区块 ID 链 + UTXO 指纹链）提供独立于签名的长期完整性。
- 附件使用 BLAKE3 提供高速验证，SHA-512 保障区块/交易层安全。
- 公钥哈希使用 SHA-384 提供 192 位安全强度，足以覆盖短暂的 UTXO 生命周期。
