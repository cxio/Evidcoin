...
- **remaining inputs** 使用 20 字节截断引用以节省空间。由于所有输入都会针对当前 UTXO 集或 UTCO 集解析（由 UTXO/UTCO 指纹保证安全），20 字节前缀已足够，实际碰撞可忽略。
...
分离首领输入使得节点对铸造竞争者的验证（择优池优选成员期间），可以不需要处理全部输入的情况下即可快速完成。
...
> 一笔交易里可以包含多笔输入，每笔输入项的私钥都需要签名交易来证明其合法。

### 4.2 Multi-Signature（多重签名）

对于同一输入源的公钥哈希：M-of-N scheme where M out of N key holders must sign.
