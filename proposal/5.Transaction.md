...
`null` 地址通常用于销毁目的地——但需配合输出项的销毁标记置位。
...
**Burn outputs**：当燃烧标记（Bit 5）置位时，输出不再进入 UTXO/UTCO 集。地址可为 `null`，锁定脚本可为空或仅含简单 `NIL`。
...
- **LotteryRetention**：来自未能完整兑奖（确认数不足）的截留部分。

### 5.3 Revenue Distribution（收益分成）

Coinbase 的收益将作为多个输出，奖励分配给多方：
...
> **注：**
> 通过输出的配置字节指定奖励目标，并严格核实锁定脚本及相关合法性。
...
**Shard verification**：在部分数据检索时，每个分片可通过含序号前缀的分片哈希与至分片树根的 Merkle 路径独立验证。
...
| 常量 | 值 | 描述 |
|----------|-------|-------------|
| MaxTxSize | 65536 bytes | 单笔交易最大尺寸 |
