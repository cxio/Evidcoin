...
- **remaining inputs** 使用 20 字节截断引用以节省空间。由于所有输入都会针对当前 UTXO 集或 UTCO 集解析（由 UTXO/UTCO 指纹保证安全），20 字节前缀已足够，实际碰撞可忽略。**注**：假设存在碰撞，后来的交易不会被通过，无法进入UTXO/UTCO集，因此安全性不受影响。
...
Level 2 (Tx8):     Tx8Hash_t       = SHA-512( Tx16Hash_1 || Tx16Hash_2 || ... )
Level 3 (Tx16):    Tx16Hash_t      = SHA-512( Tx32Hash_1 || Tx32Hash_2 || ... )
Level 4 (Tx32):    Tx32Hash_t      = SHA-512( infoHash_1 || infoHash_2 || ... )
...
// Level 3: 交易级 — 继上层分组内按交易 ID 的第16字节值分子组
// Level 4: 交易级 — 继上一子组内按交易 ID 的第32字节值分组
// 末端叶子节点：
...
type Tx8Node struct {
    Index     byte                  // 分组序位
    Hash      Hash512               // 分组哈希
    Tx16Nodes [256]*Tx16Node        // TxID 第16字节 → 子分组序列
}

type Tx16Node struct {
    Index     byte                  // 分组序位
    Hash      Hash512               // 分组哈希
    Tx32Nodes [256]*Tx32Node        // TxID 第32字节 → 子分组序列
}

type Tx32Node struct {
    Index      byte                 // 分组序位
    Hash       Hash512              // 分组哈希
    InfoHashes []Hash512            // 叶子节点列表：每个交易对应一个 InfoHash
}
...
> **注：**
> 其实 UTCO 指纹提供了第三条历史完整性耦合锁定，但逻辑类似 UTXO，因此通常仅称双链耦合保护。
...
> **注：**
> UTCO 集的转换逻辑与 UTXO 类似，但考虑的是凭信的转移次数与过期规则等因素。
...
3. **Verify**：从 Blockqs 获取哈希树验证路径（含 UTXO/UTCO 指纹），并结合用户本地区块头链进行验证：
    - 由交易数据重算 TxID。
...
- 树根与 UTXO/UTCO 指纹共同参与 `CheckRoot` 计算。
